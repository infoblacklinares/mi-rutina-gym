...
export default async function SharedRoutinePage({
  ...
  return (
    <main className="flex-1 max-w-lg w-full mx-auto">
      {/* Hero */}
      <div className="relative overflow-hidden rounded-b-[32px] px-5 pt-12 pb-8" style={{ background: "linear-gradient(160deg, #0b3557 0%, #14487a 60%, #2a6db3 100%)" }}>
      ...
      <div className="px-5 pt-5 space-y-3 pb-4">
        {routine.days.map((day) => (
          <div key={day.day} className="bg-white rounded-3xl card-shadow p-4">
            <div className="flex items-center gap-3 mb-3">
              <div className="w-10 h-10 rounded-2xl bg-[#eef3f8] flex items-center justify-center font-bold text-[#0b3557]">
                {day.day}
              </div>
              <div>
                <p className="font-semibold text-[#0c1c2c]">{day.title}</p>
                {day.extra && <p className="text-xs text-[#b46609]">⚡ {day.extra}</p>}
              </div>
            </div>
            <ul className="space-y-1.5">
              {day.exercises.map((ex) => (
                <li key={ex.name} className="flex items-center justify-between text-sm">
                  <span className="text-[#0c1c2c]">{ex.name}</span>
                  <span className="text-[#8ba0b5] text-xs">{ex.sets} × {ex.reps}</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      {/* CTA fija */}
      <div className="sticky bottom-0 bg-gradient-to-t from-[#eef3f8] via-[#eef3f8] to-transparent px-5 pt-4 pb-6">

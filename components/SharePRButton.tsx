...
export default function SharePRButton({ prs }: { prs: PR[] }) {
  ...
  async function share() {
    ...
    grad.addColorStop(0, "#0b3557");
    grad.addColorStop(0.6, "#14487a");
    grad.addColorStop(1, "#2a6db3");
    ctx.fillStyle = grad;
  ...
  return (
    <button
      onClick={share}
      disabled={busy}
      className="w-full max-w-xs rounded-2xl bg-white text-[#0b3557] font-semibold py-4 text-base card-shadow disabled:opacity-50"
    >
